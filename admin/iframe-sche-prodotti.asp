            ...
            <form method="post" action="iframe-sche-prodotti.asp?mode=1&pkid=<%=pkid%>&fk=<%=fk%>&ordine=<%=ordine%>" name="newsform">
                <table cellpadding="0" cellspacing="0" border="0" width="725">
                    ...
                    <tr>
                        ...
                        <%
												if pkid>0 then
													FkEventi=rs("FkEventi")
													if Len(FkEventi)>0 then
													arrFkEventi=split(FkEventi,", ")
													end if
												end if
												...
												%>
												  ...
												  <%end if%>
													<td>
														...
														esiste="No"
														pkid_eve=ns("PkId")
														pkid_eve=cInt(pkid_eve)
														if pkid>0 then
															'pkid=cInt(pkid)'
															if Len(arrFkEventi)>0 then
																For iLoop = LBound(arrFkEventi) to UBound(arrFkEventi)
																	FkEvento=arrFkEventi(iLoop)
																	FkEvento=cInt(FkEvento)
																	if FkEvento=pkid_eve then
																		esiste="Si"
																	end if
																Next
															else
													...
													<input name="fkeventi" type="checkbox" value="<%=pkid_eve%>" <%if esiste="Si" then%> checked<%end if%>>&nbsp;<%=ns("Titolo_1")%>
													</td>
													<%If((conta Mod 3)=2) then%>
												  </tr>
												  <tr> </tr>
