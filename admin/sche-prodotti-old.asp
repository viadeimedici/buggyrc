	...
	if mode = 1 then
		...
		if Len(Url_Old)>0 and Len(Url_New)>0 then
			'if StrComp(Url_Old, Url_New, 1)<>0 then
				...
				If FSO.FileExists(Server.MapPath("/prodotti-arredo-decorazioni/") & "\" & Url_Old) Then
					Set Documento = FSO.GetFile(Server.MapPath("/prodotti-arredo-decorazioni/") & "\" & Url_Old)
					Documento.Delete
		...
		Posizione_InEvidenza = request("Posizione_InEvidenza")
		if Posizione_InEvidenza="" then Posizione_InEvidenza=100
		rs("Posizione_InEvidenza")=Posizione_InEvidenza
                ...
                <table cellpadding="0" cellspacing="0" border="0" width="740">
                    ...
                    <tr>
                        <td class="vertspacer"><input name="Offerta" type="radio" value="0" <% if pkid > 0 then %><%if rs("Offerta")=0 then%>checked<%end if%><%else%>checked<%end if%> />
                            ...
                            &nbsp;Sì</td>
                        <td class="vertspacer"><input type="text" name="Posizione2" id="Posizione2" class="form" size="10" maxlength="5" <%if pkid>0 then%> value="<%=rs("Posizione")%>"<%end if%> /></td>
                    </tr>
