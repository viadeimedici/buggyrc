...
<%
pag_elenco="ges-preferiti.asp"
pag_scheda="ges-preferiti.asp"
voce_s="Preferiti"
voce_p="Preferiti"
...
if request("C1") = "ON" then
	...
	if pkid>0 then
		...
		sql = "Select * From Preferiti where PkId="&pkid&""
        ...
        <!--tab centrale-->
			<table border="0" cellspacing="0" cellpadding="0">
              ...
              <tr <% if t = 0 then %>class="td_alt col_secondario"<% end if %>>
                <td><a href="#"><span style="color: #c00;"><%=nrs("pkid")%>.</span></a></td>
                <td><%=Cognome%>&nbsp;<%=Nome%></td>
								<td><%=nrs("Titolo_Madre")%> - <%=nrs("Codice_Madre")%></td>
                <td><%=Left(nrs("Data"),10)%></td>
                <td align="center"><a href="<%=pag_elenco%>?pkid=<%=nrs("pkid")%>&C1=ON&ordine=<%=ordine%>&p=<%=p%>" onClick="return elimina();"><img src="immagini/delete.gif" width="16" height="16" alt="Elimina la riga" /></a></td>
               </tr>
