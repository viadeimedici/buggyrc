...
if ordine=1 then ord="Cognome ASC"
if ordine=2 then ord="Cognome DESC"
if ordine=3 then ord="Email ASC"
if ordine=4 then ord="Email DESC"


Set nrs=Server.CreateObject("ADODB.Recordset")
sql = "SELECT * FROM Iscritti ORDER BY "&ord&""
        ...
        <!--tab centrale-->
			<table border="0" cellspacing="0" cellpadding="0">
              ...
              <tr class="intestazione col_primario">
                <td width="40%"><a href="<%=pag_elenco%>?ordine=0">Cod.</a>&nbsp;Cognome&nbsp;Nome&nbsp;<a href="<%=pag_elenco%>?ordine=1">A/Z</a>&nbsp;<a href="<%=pag_elenco%>?ordine=2">Z/A</a></td>
                <td width="40%">Email&nbsp;<a href="<%=pag_elenco%>?ordine=3">A/Z</a>&nbsp;<a href="<%=pag_elenco%>?ordine=4">Z/A</a></td>
                <td width="10%">Data</td>
              ...
              <tr <% if t = 0 then %>class="td_alt col_secondario"<% end if %>>
                <td><a href="<%=pag_scheda%>?pkid=<%=nrs("pkid")%>&ordine=<%=ordine%>"><span style="color: #c00;"><%=nrs("pkid")%>.</span><%=nrs("Cognome")%>&nbsp;<%=nrs("Nome")%></a></td>
                <td><%=nrs("email")%></td>
                <td><%=Left(nrs("Data"),10)%></td>
